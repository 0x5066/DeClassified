...
Note:		Ripped from Winamp Modern, removed the VU Meter section
			...
			Edited for use in the playlist editor.
-----------------------------------------------------
---------------------------------------------------*/
...
{ 
	...
	visualizer = scriptGroup.findObject("wa.vis2");

	trigger = scriptGroup.findObject("main.vis2.trigger");
...
refreshVisSettings ()
{
	currentMode = getPrivateInt(getSkinName(), "PL Visualizer Mode", 1);
...
setVis (int mode)
{
	setPrivateInt(getSkinName(), "PL Visualizer Mode", mode);
	if (mode == 0)
	{
